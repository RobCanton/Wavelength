...
class MusicPlayerManager: NSObject {
    ...
    static let nowPlayingChanged = NSNotification.Name("nowPlayingChanged")
    ...
    var isPlaying:Bool {
        get {
            return musicPlayerController.playbackState == .playing
        }
    }
    
    private(set) var queue: [Any] = []
    ...
    func beginPlayback(queueIDs requestedQueue:[String]) {
        if requestedQueue.count > 0 {
            print("Overriding existing queue")
        }
        queue = requestedQueue
        var strQueue = [String]()
        for item in queue {
            if let mediaItem = item as? MPMediaItem {
                strQueue.append("\(mediaItem.persistentID)")
            } else if let mediaItem = item as? MediaItem {
                strQueue.append(mediaItem.identifier)
            } else if let itemID = item as? String {
                strQueue.append(itemID)
            }
        }
        DispatchQueue.global(qos: .background).async {
            self.musicPlayerController.setQueue(with: strQueue)
            self.musicPlayerController.play()
        }
    }
    
    func beginPlayback(queue requestedQueue:[MPMediaItem]) {
        if requestedQueue.count > 0 {
            print("Overriding existing queue")
        }
        queue = requestedQueue
        ...
        for item in queue {
            if let mediaItem = item as? MPMediaItem {
                strQueue.append("\(mediaItem.playbackStoreID)")
            } else if let mediaItem = item as? MediaItem {
                strQueue.append(mediaItem.identifier)
            } else if let itemID = item as? String {
                strQueue.append(itemID)
    ...
    func handleMusicPlayerControllerNowPlayingItemDidChange() {
        NotificationCenter.default.post(name: MusicPlayerManager.nowPlayingChanged, object: nil)
    }
